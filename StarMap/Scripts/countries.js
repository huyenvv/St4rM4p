...
$(document).ready(function () {
    ...
    $(countryId).change(function () {
        printState("City", getStateIndex($(this).find('option:selected').attr("value")));
    });
});
function printCountry(countryId) {
    // given the id of the <select> tag as function argument, it inserts <option> tags
    var optionStr = document.getElementById(countryId);
    optionStr.length = 0;
    optionStr.options[0] = new Option(starmap.CountrySelect, '');
    optionStr.selectedIndex = 0;
    for (var i = 0; i < country_arr.length; i++) {
        optionStr.options[optionStr.length] = new Option(country_arr[i], country_arr[i]);
    }
}
function getStateIndex(country) {
    var stateIndex = 0;
    for (var index = 0; index < country_arr.length; index++) {
        if (country === country_arr[index]) {
            stateIndex = index + 1; //because country_arr start with s[0] = ""
            break;
        }
    }
    return stateIndex;
...
function printState(stateId, stateIndex) {
    var optionStr = document.getElementById(stateId);
    optionStr.length = 0;	// Fixed by Julian Woods
    optionStr.options[0] = new Option(starmap.CitySelect, '');
    optionStr.selectedIndex = 0;
